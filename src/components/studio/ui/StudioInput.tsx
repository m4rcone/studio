    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <input
          ref={ref}
          id={id}
          className={`st-focus-ring rounded-(--st-radius) border border-(--st-border) bg-(--st-bg-input) px-4 py-3 text-base text-(--st-text) shadow-[inset_0_1px_0_rgb(255_255_255_/_0.7)] transition-[border-color,box-shadow,background-color] placeholder:text-(--st-text-muted) sm:text-sm ${error ? "border-(--st-danger)" : "focus:border-(--st-border-strong) focus:bg-(--st-bg-surface) focus:shadow-[0_0_0_4px_var(--st-focus-soft)]"} ${className}`}
          {...props}
        />
        {error && (
